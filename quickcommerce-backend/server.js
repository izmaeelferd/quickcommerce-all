...
app.post('/api/admin/banners', authMiddleware, adminMiddleware, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            'INSERT INTO banners (image_url, title, link_url) VALUES ($1,$2,$3) RETURNING *',
...
// ========== PUBLIC BANNER ROUTE (for customer app) ==========
app.get('/api/banners', async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM banners WHERE is_active = true ORDER BY id DESC');
        res.json(result.rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
